...
func generateBlockMapping() (map[int]string, map[string]int) {
	...
	err := json.Unmarshal([]byte(legacy), &temp)
	if err != nil {
		fmt.Println("Err while loading legacy blocks")
	}
	...
	for k, v := range temp.Blocks {
		...
		if err != nil {
			fmt.Println("Err while loading legacy block id", k)
			continue
		}
		data, err := strconv.Atoi(s[1])
		if err != nil {
			fmt.Println("Err while loading legacy block data", k)
			continue
		}

		bigId := GetLegacyFromTypeData(id, data)
		legacyToString[bigId] = v
		stringToLegacy[v] = bigId
	}

	return legacyToString, stringToLegacy
}

func GetNameFromLegacy(legacy int) string {
	if val, ok := legacyToString[legacy]; ok {
		return val
	} else {
...
func GetLegacyFromName(name string) int {
	if val, ok := stringToLegacy[name]; ok {
		return val
	} else {
		return 0
	}
...
func GetLegacyMapping() map[int]string {
	return legacyToString
}

func GetLegacyFromTypeData(typ int, data int) int {
	return typ << 4 | (data & 0xF)
}
